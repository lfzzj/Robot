...
public class WifiAdminUtils {
     ...
     * @return<br>
     */
    public boolean isConnect(WifiBean result) {
        if (result == null) {
            return false;
        }

        mWifiInfo = mWifiManager.getConnectionInfo();
        String g2 = "\"" + result.getWifiName() + "\"";
    ...
    public boolean connectSpecificAP(WifiBean scan) {
        ...
        for (WifiConfiguration w : list) {
            ...
            if (w.BSSID != null && w.BSSID.equals(scan.getBSSID())) {
    ...
    public WifiConfiguration CreateWifiInfo(WifiBean scan, String Password) {
        WifiConfiguration config = new WifiConfiguration();
        config.hiddenSSID = false;
        config.status = WifiConfiguration.Status.ENABLED;

        if (scan.getCapabilities().contains("WEP")) {
            config.allowedKeyManagement.set(WifiConfiguration.KeyMgmt.NONE);
            config.allowedAuthAlgorithms
                    .set(WifiConfiguration.AuthAlgorithm.OPEN);
            config.allowedGroupCiphers
            ...
            config.SSID = "\"" + scan.getWifiName() + "\"";

            config.wepTxKeyIndex = 0;
            config.wepKeys[0] = Password;
            // config.preSharedKey = "\"" + SHARED_KEY + "\"";
        } else if (scan.getCapabilities().contains("PSK")) {
            //
            config.SSID = "\"" + scan.getWifiName() + "\"";
            config.preSharedKey = "\"" + Password + "\"";
        } else if (scan.getCapabilities().contains("EAP")) {
